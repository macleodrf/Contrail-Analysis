...
def parse_time_input(time_str, allow_any_time=False, suggested_time=None):
    try:
        ...
        try:
            ...
            print("Error: Invalid time format. See User Guide.")
            if suggested_time:
    ...
    if not allow_any_time:
        ...
        if not ((obs_hour == 0 or obs_hour == 12) and obs_minute <= 15):
            print("Error: Obs time must be 00Z or 12Z. See User Guide.")
            if suggested_time:
                print(f"Suggested time: {suggested_time}")
            return None, time_str
    return dt, time_str
...
def check_time_proximity(obs_dt, img_dt, suggested_time):
    ...
    if time_diff > timedelta(hours=6):
        return False, f"Obs time is more than 6 hours from image time. Suggested time: {suggested_time}"
    return True, ""
...
try:
    ...
    if not all(col in df.columns for col in ['P', 'T', 'Td']):
        raise ValueError("Data must include PRES, TEMP, DWPT columns. See User Guide.")
    if not df['P'].between(200, 300).any():
        raise ValueError("Data must include levels between 200 and 300 hPa.")
